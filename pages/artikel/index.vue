<template>
  <div style="margin-top: 100px; margin-bottom: 20px" class="ptserrifText">
    <v-container>
      <div class="mb-3">
        <Subheading2 :subtext="'Artikel Terbaru'" />
      </div>
      <v-text-field variant="outlined" style="margin-top: 12px; box-shadow: 0px 0px 4px #e2efff; padding: 28px 20px 0px 20px" v-model="searchQuery" placeholder="cari artikel disini" prepend-icon="fa fa-search"> </v-text-field>
      <v-row style="display: flex; justify-content: center">
        <v-col :cols="12" :lg="6" v-for="(article, index) in currentResults" :key="index">
          <v-card style="height: 220px; border-radius: 20px" v-if="artikelData">
            <v-row>
              <v-col :cols="6">
              ...
              <v-col :cols="6">
                <div style="padding: 2px">
                  ...
                  <div>
                    ...
                    <p class="text-content text-wrap cairoText" v-dompurify-html="article.content" style="color: #5a524c; line-height: 150%"></p>
                  </div>
                  <div style="margin-top: 16px">
                    <NuxtLink :to="`/artikel/${article.slug}`" class="cairoText" style="font-size: 18px; color: #2b6956">Baca Selengkapnya</NuxtLink>
                  </div>
                </div>
              </v-col>
            </v-row>
...
<!-- <template>
  <div style="margin-top: 100px; margin-bottom: 20px" class="ptserrifText">
    <v-container>
      ...
      <v-text-field variant="outlined" style="margin-top: 12px; box-shadow: 0px 0px 4px #e2efff; padding: 28px 20px 0px 20px" v-model="searchQuery" placeholder="cari artikel disini" prepend-icon="fa-brands fa-twitter"> </v-text-field>
      <v-row>
        <div class="d-flex flex-wrap">
          <v-card class="mx-auto" max-width="344" v-for="(article, index) in currentResults" :key="index">
            <v-img :src="article.imgLink" height="200px" cover></v-img>
...
const fetchData = async () => {
  try {
    const response = await fetch('https://api.imavi.org/imavi/articles/get-all', {
      headers: {
    ...
    artikelData.value = hasil;
  } catch (error) {
    console.error('Error fetching data:', error);
  }
};
await fetchData();
// console.log(artikelData.value);
totalPages = ref(Math.ceil(artikelData.value.length / articlePerPage));
...
const displayedArticles = computed(() => {
  const startIndex = (currentPage.value - 1) * articlePerPage;
  const endIndex = startIndex + articlePerPage;
  return artikelData.value.slice(startIndex, endIndex);
});
const initialResults = computed(() => {
  if (Array.isArray(displayedArticles.value)) {
...
const searchResults = computed(() => {
  if (Array.isArray(artikelData.value)) {
    return artikelData.value.filter((image) => image.title.toLowerCase().includes(searchQuery.value.toLowerCase()));
  } else {
    return [];
  }
});
...
const currentResults = computed(() => {
  ...
  if (searchQuery.value === '') {
    ...
    totalPages.value = Math.ceil(result.length / articlePerPage);
