    ...
    <v-row class="d-flex justify-center">
      <v-col :cols="12" :lg="10" :md="6">
        <div style="margin-top: 100px">
          ...
          <div class="content2" v-html="artikelDetail.content"></div>
        </div>
        <div class="text-center mt-4">
          <NuxtLink to="/berita"><Button :btnTitle="'Kembali'" /></NuxtLink>
        </div>
...
const fetchData = async () => {
  try {
    const response = await fetch(`https://api.imavi.org/imavi/news/view/${useRoute().params.slug}`, {
      headers: {
